...
def update_packet(prev_packet, new_payload):
    ...
    p[Dot11EltVendorSpecific][len([prev_packet[Dot11EltVendorSpecific]])-1].payload = new_tag_vendor_dji
    return p
...
def thread_send(packet):
    ...
    while is_event == 0:
        count += 1
        try:
            sendp(packet[1], iface=interface, verbose=1, loop=0, count=1)
            #time.sleep(1)
...
def one_drone():
    ...
    original_packet = telemetry_packet.copy()
    packet_list = []
        ...
        while 1:      
            try:
                ...
                for event in events:
                    ...
                    axis, value = event.code.split("_")[1], event.state
                    ...
                    ## Dont update packet in case of this events
                    if "SYN" in axis or value == 0:
                        updated = False
                        continue
                    if (event.code == "ABS_X" or event.code == "ABS_Y" or event.code == "ABS_RX" or event.code == "ABS_RY") and abs(event.state) < 15000:
                        ...
                        event_counter["X"] += 1 
                        if event_counter["X"] % 10 != 0 :
                            updated = False
                            continue
                        event_counter["X"] = 0


                        drone.longitude = float("{:.6f}".format(drone.longitude +  float("{:.4f}".format(float(value_sign/ 1000))))) # To modify the 4 decimal digit
                        ...
                        event_counter["Y"] += 1 
                        if event_counter["Y"] % 10 != 0 :
                            updated = False
                            continue
                        event_counter["Y"] = 0                      
                        drone.latitude = float("{:.6f}".format(drone.latitude + float("{:.4f}".format(float(value_sign / 1000))) * (-1))) # invert sign: left should be positiv and right negative. Controller returns the other way
                    ...
                    if axis == "HAT0X": # Modify Longitude
                        ...
                        drone.longitude = drone.longitude +  float("{:.4f}".format(float(value_sign/ 1000))) 
                        drone.v_east =  (drone.v_east + 100 * value_sign)   # All the time will increase the speed and the pointer will point to that direction
                ...
                if restart is True:
                    packet_list[1] = original_packet.copy()
                    restart = False
                    print("RESET")
                else:

                    #f.write(packet_list[1][Dot11EltVendorSpecific][len([packet_list[1][Dot11EltVendorSpecific]])-1].show(dump=True))                
                    packet_list[1] = create_packet(beacon_base_packet, new_payload)
                    print("UPDATED")
                #print(packet_list[1][Dot11EltVendorSpecific].show(dump=True))
